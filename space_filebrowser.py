...
class FILE_HOME(bpy.types.Operator):
    bl_idname = "file.go_home"
    bl_label = "Go to home directory"

    def execute(self, context):
        
        #find binary path from bpy then truncate the last slash    
        bin_path = bpy.app.binary_path.rsplit('\\', 1)[0]
        
        print(bin_path)
        
        context.space_data.params.directory = bin_path
        
        return {'FINISHED'}

bpy.utils.register_class(FILE_HOME)
...
class FILEBROWSER_HT_header(Header):
    ...
    def draw(self, context):
        ...
        row.operator("file.go_home", text="", icon='WORLD_DATA')
